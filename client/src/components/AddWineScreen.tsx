...
const AddWineScreen: React.FC<AddWineScreenProps> = ({ onBack, apiUrl }) => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setForm(prev => ({ ...prev, imageFile: compressed, imageUrl: URL.createObjectURL(compressed) }));
    } catch {
      setErrorMessage(true); setTimeout(() => setErrorMessage(false), 2000);
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name || !form.rebsorte || !form.farbe || !form.preis ||
        form.kauforte.length === 0 || !form.kategorie || !form.unterkategorie ||
        form.geschmack.length === 0 || form.bewertung === 0) {
      setErrorMessage(true); setTimeout(() => setErrorMessage(false), 2000); return;
    }
    setIsSaving(true); setErrorMessage(false);
    try {
      ...
      if (form.imageFile) {
        ...
        const uploadResponse = await imagekit.upload({
          file: form.imageFile, fileName: createUrlSafeFileName(form.name, form.rebsorte),
          folder: '/wines', ...auth,
        });
    ...
    <div className="App">
      ...
      <main className="flex-1 p-6 flex flex-col items-center gap-6">
        ...
        <section className="glass-card">
          ...
          {['Sommer','Winter'].map(s => (
            <label key={s} className="unterkategorie-label" style={{ cursor:'pointer' }}>
              <input type="checkbox" checked={form.saison === s}
        ...
        <section className="glass-card bewertung-card">
          ...
          <div className="flex flex-row gap-2 flex-nowrap">
            {[1,2,3,4,5].map(star => (
              <svg key={star} className="cursor-pointer flex-shrink-0"
                style={{
                  width:32, height:32,
      ...
      <footer className="footer">
        <div style={{ display:'flex', justifyContent:'flex-end', width:'100%' }}>
          <button className="footer-btn" onClick={handleSave} disabled={isSaving || isUploading}>
            Speichern
          </button>
        </div>
      </footer>

      {successMessage && <div className="snackbar success">Wein erfolgreich hinzugefügt!</div>}
      {errorMessage   && <div className="snackbar error">Bitte fülle alle Pflichtfelder aus!</div>}
    </div>
  );
};
